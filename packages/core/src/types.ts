...
/** 单个三合/半合识别结果 — 只算结构事实，是否「真成局起作用」留 AI 断 */
export interface SanHeMatch {
  /** 局型：'三合局'=三支全现，'半合'=两支现 */
  type: '三合局' | '半合';
  /** 化气五行（如「水」） */
  wuxing: string;
  /** 现于卦中的地支（按 申子辰 本组顺序，仅含已现支） */
  zhis: string[];
  /** 携带这些地支的爻位（1-based，升序；同支多爻则全列） */
  positions: number[];
  /** 是否含中神（帝旺/四正支 子午卯酉）：半合含中神力强，缺中神为拱合多需引动 */
  has_center: boolean;
  /** 一句话说明 */
  note: string;
}

/** 三合局/半合 — 扫六爻地支识别成局结构；化气方向算死，成局取舍留 AI */
...
export interface HexagramResult {
  ...
  /** 三合局/半合（扫六爻地支识别成局结构）；恒附加（纯卦象固有属性） */
  san_he?: SanHe | null;
  // 时间维度断卦属性
