...
// 地支
export const ZHIS: readonly string[] = [
  '子',
  '丑',
  '寅',
  '卯',
...
/**
 * 三合局表：申子辰合水、亥卯未合木、寅午戌合火、巳酉丑合金。
 * 三支虽按索引等距（每组间隔 4），但「哪组化哪行」无规律，必须显式表。
 * center 为中神（帝旺，即四正支 子午卯酉）：半合含中神者力强，缺中神者多需引动，留 AI 断。
 */
export const SANHE_MAP: ReadonlyArray<{
  zhis: readonly [string, string, string];
  center: string;
  wuxing: string;
}> = [
  { zhis: ['申', '子', '辰'], center: '子', wuxing: '水' },
  { zhis: ['亥', '卯', '未'], center: '卯', wuxing: '木' },
  { zhis: ['寅', '午', '戌'], center: '午', wuxing: '火' },
  { zhis: ['巳', '酉', '丑'], center: '酉', wuxing: '金' },
];
