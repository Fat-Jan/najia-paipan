...
import { calcDayDynamics, calcYingQi, calcSanHe } from '../src/time-analysis.js';
...
describe('三合局 / 半合（calcSanHe）', () => {
  // 构造三字 qinx「干支+五行」，地支放 index 1（与生产 gz5x 输出同格式）；
  // 五行字仅占位（calcSanHe 只读地支），故用对应正确五行保持真实。
  ...
  const mkQinx = (zhis: string[]): string[] => zhis.map((z) => `甲${z}${ZHI_WX[z]}`);
  ...
  it('四大三合局三支全现 → 化气五行正确', () => {
    ...
    for (const [zhis, wuxing] of cases) {
      const r = calcSanHe(mkQinx(zhis));
      const quan = r.matches.find((m) => m.type === '三合局' && m.wuxing === wuxing);
      expect(quan).toBeDefined();
      expect(quan?.zhis.length).toBe(3);
      expect(quan?.has_center).toBe(true); // 全现必含中神
    }
  });

  it('三合局爻位按地支出现位置升序列出', () => {
    // 申(爻1) 子(爻2) 辰(爻3)
    const r = calcSanHe(mkQinx(['申', '子', '辰', '巳', '巳', '巳']));
    const quan = r.matches.find((m) => m.type === '三合局');
    expect(quan?.positions).toEqual([1, 2, 3]);
  });

  it('含中神的半合（两支现，含子午卯酉）→ has_center=true', () => {
    // 申子（缺辰）含中神子 → 半合水
    const r = calcSanHe(mkQinx(['申', '子', '巳', '巳', '巳', '巳']));
    const ban = r.matches.find((m) => m.type === '半合' && m.wuxing === '水');
    expect(ban).toBeDefined();
    expect(ban?.has_center).toBe(true);
    expect(ban?.zhis.sort()).toEqual(['子', '申'].sort());
  });

  it('缺中神的半合（拱合，如申辰）→ has_center=false', () => {
    // 申辰（缺子）不含中神 → 拱合水
    const r = calcSanHe(mkQinx(['申', '辰', '巳', '巳', '巳', '巳']));
    const ban = r.matches.find((m) => m.type === '半合' && m.wuxing === '水');
    expect(ban).toBeDefined();
    expect(ban?.has_center).toBe(false);
    expect(ban?.note).toContain('拱合');
  });

  it('只现一支 → 不成局', () => {
    const r = calcSanHe(mkQinx(['申', '寅', '寅', '寅', '寅', '寅']));
    expect(r.matches).toEqual([]);
    expect(r.note).toBe('');
  });

  it('三支全现时不再重复输出其子半合（只判三合局）', () => {
    const r = calcSanHe(mkQinx(['申', '子', '辰', '巳', '巳', '巳']));
    const waterMatches = r.matches.filter((m) => m.wuxing === '水');
    expect(waterMatches.length).toBe(1);
    expect(waterMatches[0].type).toBe('三合局');
  });

  it('同支多爻 → positions 列全部', () => {
    // 子现于爻2、爻5；申爻1 → 申子半合，positions 含 1/2/5
    const r = calcSanHe(mkQinx(['申', '子', '巳', '巳', '子', '巳']));
    const ban = r.matches.find((m) => m.type === '半合' && m.wuxing === '水');
    expect(ban?.positions).toEqual([1, 2, 5]);
  });
});

// 集成：真实 compile() 驱动 san_he，堵「手造数据 vs 生产三字格式」脱节洞。
// 乾为天纳甲：爻1子 爻2寅 爻3辰 爻4午 爻5申 爻6戌 → 同时含申子辰(水)与寅午戌(火)两个三合局。
describe('集成：真实 compile() 驱动三合局（防数据格式脱节）', () => {
  it('乾为天恒算 san_he：识别申子辰水局 + 寅午戌火局', () => {
    const r = compile({ params: [1, 1, 1, 1, 1, 1] }); // 乾为天，六爻地支 子寅辰午申戌
    expect(r.san_he).toBeDefined();
    const quanju = r.san_he?.matches.filter((m) => m.type === '三合局') ?? [];
    const byWx = Object.fromEntries(quanju.map((m) => [m.wuxing, m]));
    // 水局：申(爻5) 子(爻1) 辰(爻3)
    expect(byWx['水']).toBeDefined();
    expect(byWx['水'].positions).toEqual([1, 3, 5]);
    // 火局：寅(爻2) 午(爻4) 戌(爻6)
    expect(byWx['火']).toBeDefined();
    expect(byWx['火'].positions).toEqual([2, 4, 6]);
  });

  it('compile 输出 qinx 为三字且 calcSanHe 取 [1] 地支成局（bug 态会取成五行字而不成局）', () => {
    const r = compile({ params: [1, 1, 1, 1, 1, 1] });
    expect(r.qinx.every((gz) => gz.length === 3)).toBe(true);
    // san_he 恒算非空即证明取的是地支而非五行（五行字不构成三合组）
    expect(r.san_he?.matches.length ?? 0).toBeGreaterThan(0);
  });

  it('坤为地恒算 san_he：识别亥卯未木局 + 巳酉丑金局（覆盖另两局）', () => {
    // 坤为地纳甲：爻1未 爻2巳 爻3卯 爻4丑 爻5亥 爻6酉
    // 亥(爻5)卯(爻3)未(爻1) 三合木；巳(爻2)酉(爻6)丑(爻4) 三合金
    const r = compile({ params: [2, 2, 2, 2, 2, 2] }); // 坤为地
    const quanju = r.san_he?.matches.filter((m) => m.type === '三合局') ?? [];
    const byWx = Object.fromEntries(quanju.map((m) => [m.wuxing, m]));
    expect(byWx['木']).toBeDefined();
    expect(byWx['木'].positions).toEqual([1, 3, 5]);
    expect(byWx['金']).toBeDefined();
    expect(byWx['金'].positions).toEqual([2, 4, 6]);
    // 结构守卫：每条 note 非空
    for (const m of r.san_he?.matches ?? []) {
